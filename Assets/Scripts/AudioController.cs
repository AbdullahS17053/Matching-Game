...
public class AudioController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // Load saved state
        isAudioOn = PlayerPrefs.GetInt(AudioStateKey, 1) == 1;
    ...
    {
        StartCoroutine(GameManager.Instance.PlayTapEffect(() =>
        {
            isAudioOn = !isAudioOn;
    ...
    private void UpdateAudioState()
    {
        ...
        if (buttonImage != null)
        {
            buttonImage.sprite = isAudioOn ? soundOnSprite : soundOffSprite;
        }
    }

    private void TryFindAudioBtn()
    {
        if (!audioToggleButton)
        {
            GameObject btnObj = GameObject.FindGameObjectWithTag("AudioButton");
            if (btnObj)
            {
                audioToggleButton = btnObj.GetComponent<Button>();
                buttonImage = audioToggleButton.GetComponent<Image>();
